...
\ProvidesPackage{asyfig}[2010/02/27 v0.1d
...
%\pdfoutput=0 
%%  (remember to delete asyfig_.tex if it's previously been generated for PDF mode)

\documentclass[12pt,twocolumn]{article}
\usepackage[sc]{mathpazo}
\usepackage{asyfig}

% If you're going to be defining macros with arguments inside
% |\asyProcessPreamble|, you'll need to temporarily change the \#
% character to be a letter (catcode 11).  it's usually a parameter
% (catcode 6), but to the main \LaTeX run a parameter \# would look
% like an invalid |\asyProcessPreamble| parameter.

\catcode`\#=11
\renewcommand\asyProcessPreamble{
    \documentclass{article}

% macro definitions in \asyProcessPreamble need not be identical to the
% ``real'' definitions, but the better they are, the better Asymptote can
% estimate their final size.

    \newcommand\myLabel[1]{\textbf{#1}}
}
\catcode`\#=6

\newcommand\myLabel[1]{\textit{#1}}
\begin{document}
A boxed graphic:\\
...
draw( "{\myLabel{Anti-resonance}}" , align=W, (4,-2) );
\end{filecontents*}

\begin{filecontents*}{asyprocess.sh}
#!/bin/sh
#
# Process Asymptote files externally for asyfig inclusion.
#
# usage: asyprocess [OPTIONS] -- [ASY-FILE] ...
#
# All options are passed directly to LATEX except for:
# --preamble=STR  the value of which is used for the LaTeX preamble.
LATEX=pdflatex
...
for file in $*; do
  ...
  $LATEX -shell-escape -jobname="${file}-comp" -interaction=batchmode \
    $LATEX_OPTIONS \
    "$PREAMBLE \usepackage{asyprocess}\ProcessAsy \
     \begin{document}\ShowAsy\end{document}" || exit 1
done

exit 0
\end{filecontents*}
% $ (close dollar signs, if necessary, from asyprocess.sh for emacs' latex-mode)
...
This package works with Asymptote v1.78 (and later, probably).
...
\paragraph{Processing the graphic automatically}
But before the graphic can be placed into the document it must be processed. If the \texttt{asy} graphic has not been processed, or if the \texttt{asy} file is \emph{newer} than its processed graphic, then this package will attempt to do the processing automatically.  This may require the \texttt{-shell-escape} option or its equivalent for your flavor of \LaTeX.  To turn off automatic processing, load the package with the \texttt{[process=none]} package option. Alternatively, to re-process \emph{all} \texttt{asy} graphics, use \texttt{[process=all]} instead.

By default, the graphic is processed using the \texttt{article} class.  If you are using a different class or have special macros that you use in your figure labels, you may want to redefine |\asyProcessPreamble| so Asymptote knows how big labels are going to be, etc.  You could accomplish the same thing with \texttt{texpreamble} in your |.asy| files, but |\asyProcessPreamble| lets you place that code in your document preamble.
\begin{lstlisting}
\usepackage{asyfig}
\renewcomannd\asyProcessPreamble{
  \documentclass{memoir}
  \newcommand\fancyLabel{\large\it}
}
\end{lstlisting}

Automatic processing is already an improvement over the stock Asymptote inline mode, which is equivalent to \texttt{[process=all]}.  However, sticking to automatic processing would still require a full document rebuild before you could see the effects of a graphics change.  If you want to tighten the feedback loop a bit more, you can process your graphics externally.

\paragraph{Processing the graphic externally}
...
By saving the script above into (say) |asyprocess.sh| and making it executable, an individual \texttt{asy} graphic can be processed by running (following from the running example)
\begin{lstlisting}
PREAMBLE='\documentclass{article} \newcommand\myLabel[1]{\textbf{#1}}'
asyprocess.sh --preamble="$PREAMBLE" -- example/frf
\end{lstlisting}
% $ (close dollar signs, if necessary, from asyprocess.sh call for emacs' latex-mode)
The |--preamble| option fills the same role that |\asyProcessPreamble| does for automatic processing.
